...
class Index extends Base
{
    ...
    public function index()
    {
        ...
        foreach ($runningMachine as $key => $item) {
            if (array_key_exists($item['product'], $store)) {
        ...
        foreach ($store as $key => $item) {
            $store[$key]['total_profit'] = Db::table('clock')->alias('c')
            ->join('machine m', 'm.mid = c.mid')
            ->where('m.product', '=', $key)
            ->sum('c.money');
        }
    ...
    {
        // 查询文章
        $article = Db::table('article')->where('type', '=', 9)->find();
        // 提交
        if ($req->isPost()) {
            // 获取编号
            $id = $req->param('id');
            ...
            // 判断结果
            if (empty($bool)) {
                $this->error('很抱歉、关于我们编辑失败请重试！');
                exit;
            }
            // 操作成功
            $this->success('恭喜您、操作成功！', '/admin/index/about.html');
        }
        // 还没有内容
        if (empty($article)) {
            $article = [
    ...
    public function fund(Request $req)
    {
        ...
        if ($req->isPost()) {
            ...
            // 保存设置
            try {
                Configure::set('hello.fund', ['content' => $content, 'date' => $this->timestamp]);
            } catch (\Exception $e) {
                $this->error($e->getMessage());
                exit;
            }
            // 操作成功
            $this->success('恭喜您、操作成功！', '/admin/index/fund.html');
        }
        // 还没有内容
        if (empty($fund)) {
            $fund = [
        ...
        } else if (!array_key_exists('content', $fund)) {
            $fund['content'] = '';
        }
        // 显示页面
