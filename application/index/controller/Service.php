...
class Service extends Base
{
    ...
    public function sms(Request $req)
    {
        ...
        // 查询次数
        if (array_key_exists('hour', $config) && !empty($config['hour'])) {
        ...
        } else if (array_key_exists('juhe', $config)) {
            $result = $this->juhe_sms($mobile, $config['juhe'], 'verify', $data[0]);
        } else {
     ...
     * 聚合 - 短信
     */
    private function juhe_sms($mobile, $config, $type, $value = '')
    {
        $url = $config['url'];
        $param = [
            'key'       =>  $config['appkey'],
