...
class Store extends Base
{
     ...
     * 赠送矿机
     */
    public function give($id, $username)
    {
        // 查询矿机
        $product = null;
        if (is_array($id)) {
            ...
            if (empty($product)) {
                throw new \think\Exception("很抱歉、指定的矿机不存在！");
            }
        }
        // 创建矿机
    ...
    public function buy(Request $req)
    {
        ...
        $frozen = $currencys[2];
        $frozen['key'] = 2;
        if (!empty(config('hello.currencys.3.enable')) && $currency == 3) {
        ...
        // 余额判断
        if (!$activation && $user['wallet'][$available['field']] < $product[$price_field]) {
            return json([
                'code'      =>  507,
                'message'   =>  '很抱歉、您的' . $available['name'] . '不足！'
            ]);
        }
        // 不能买自己的东西
            ...
            if ($product['catalog'] == 1) {
                ...
                if (!empty($machines)) {
                    ...
                    if ($count >= $product['limit']) {
                        return json([
                            'code'      =>  508,
                            'message'   =>  '很抱歉、该矿机每人限量' . $product['limit'] . '台！'
                        ]);
                    }
                }
            } else {
        ...
        try {
            ...
            // 使用激活码
            $code = $req->param('code');
            if ($activation && $product['catalog'] == 1) {
                // 判断激活码
            ...
            if ($product['catalog'] == 1) {
                ...
                // 互助基金
                (new Fund())->donation($username, $product['id'], $product[$price_field]);
                // 操作日志
                $this->log(32);
            }
            // 提交事务
    ...
    public function order(Request $req)
    {
        ...
        if ($req->isPost()) {
            ...
            $query = Db::table('order')->alias('o')
                    ...
                    ->field('o.oid, o.status, o.title, o.price, o.charge, o.address, o.currency, o.express, p_seller.username AS seller_username, p_seller.nickname AS seller_nickname, p_buyer.username AS buyer_username, p_buyer.nickname AS buyer_nickname, o.update_at AS date');
            // 条件：按类目搜索
            $catalog = $req->param('catalog/d');
            if (!empty($catalog)) {
            ...
            foreach ($data as $key => $item) {
                ...
                // 支付货币
                $item['currency'] = config('hello.currencys.' . $item['currency'] . '.name') ?: config('hello.unit');
                // 收货信息
